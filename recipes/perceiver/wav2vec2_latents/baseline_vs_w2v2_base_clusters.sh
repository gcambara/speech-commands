...
GPU=3
SAVE_DIR=recipes/perceiver/wav2vec2_latents/results/baseline_vs_w2v2_base_clusters
export CUDA_VISIBLE_DEVICES=$GPU
cd ../../../

NUM_LATENTS=20
echo "Running Perceiver base frozen experiment with $NUM_LATENTS latents..."
...
for SEED in {1..10}
do
    python train.py --config $CFG --run_dir $RUN_DIR --classifier $CLASSIFIER --prc_num_latents $NUM_LATENTS --prc_freeze_latents $FREEZE --clusterize_latents $CLUSTERIZE --seed $SEED
done
...
echo "Running Perceiver base unfrozen experiment with $NUM_LATENTS latents..."
CLASSIFIER=perceiver
